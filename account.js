(function () {
    ...
    let adminSelectedCentre = null; // ID ou 'independent'
    let adminSelectedTrainer = null; // ID
    ...
    async function loadAccountDetails() {
        ...
        try {
            ...
            if (!userData) return;
            if (userData.email === 'lucas.seraudie@gmail.com') document.getElementById('tab-admin').style.display = 'flex';
            ...
            if (userData.role === 'formateur' && userData.organisation) {
                ...
                pName.textContent = `Plan ${displayPlan} (via ${userData.organisation.name})`;
                ...
                pDesc.textContent = `Gestionnaire de "${userData.organisation.name}".`;
                document.getElementById('tab-invitations').style.display = 'flex';
    ...
    function renderAdminHierarchy() {
        ...
        if (adminSelectedCentre === 'independent') {
            // Tous les users (sauf etudiants) qui n'ont pas d'organisation et ne sont pas owner d'une org
            // Note: un 'owner' a une org. un 'formateur' a une org. un 'user' (indep/free/promo) n'en a pas.
            filteredTrainers = adminData.users.filter(u => 
                u.role !== 'etudiant' && 
                !u.organisation && 
                !u.is_owner // Exclure les owners d'orgs
            );
        } else if (adminSelectedCentre) {
            // Formateurs de l'organisation sélectionnée + le propriétaire
            filteredTrainers = adminData.users.filter(u => 
                (u.organisation && u.organisation._id === adminSelectedCentre && u.role === 'formateur') ||
                (u.organisation && u.organisation._id === adminSelectedCentre && u.is_owner) // Le proprio est aussi un formateur
            );
        }
        ...
        if (adminSelectedTrainer) {
            // Étudiants de ce formateur spécifique
            filteredStudents = adminData.users.filter(u => u.role === 'etudiant' && u.createdBy && u.createdBy._id === adminSelectedTrainer);
        } else if (adminSelectedCentre) {
            // Tous les étudiants du centre (agrégé)
            // Il faut trouver tous les formateurs du centre, puis tous les étudiants créés par eux
        ...
        if (!adminSelectedCentre) {
            ...
            filteredStudents.forEach(stu => {
                // Trouver son formateur pour l'affichage
    ...
    window.selectAdminCentre = function(centreId) {
        ...
        adminSelectedTrainer = null; // Reset trainer selection when center changes
        renderAdminHierarchy();
    };
    ...
    async function handleAdminDeleteScenario(patientId) {
        showDeleteConfirmation("Supprimer ce scénario définitivement ?", async () => {
            try {
                ...
                showCustomAlert("Succès", res.message);
                loadAdminDashboard(); // Rafraîchir la liste
            } catch (err) { showCustomAlert("Erreur", err.message); }
        });
    }
