(function() {
    ...
    function addObservation(data, fromLoad = false) {
        ...
        // Correction XSS : Sanitize author before insertion
        const safeAuthor = DOMPurify.sanitize(author).toUpperCase();

        item.innerHTML = `
            <div class="timeline-dot dot-rose"></div>
            <div class="flex justify-between items-start">
                <h3 class="font-semibold text-gray-800">${formattedDate} - ${safeAuthor}</h3>
        ...
        `;
        item.querySelector('p').textContent = text; // Safe by default
    ...
    function addTransmission(data, fromLoad = false) {
        ...
        // Correction XSS : Sanitize author
        const safeAuthor = DOMPurify.sanitize(author).toUpperCase();
        ...
        // Correction XSS : Sanitize the formatted text before inserting
    ...
    function addPrescription(data, fromLoad = false) {
        ...
        // Correction XSS : Sanitize input fields
        ...
        newRow.innerHTML = `
            ...
            <td class="p-2 text-left align-top min-w-[144px]">${safePosologie}</td>
            <td class="p-2 text-left align-top min-w-[96px]">${safeVoie}</td>
    ...
    function addCareDiagramRow(data) {
        ...
        // Correction XSS : Sanitize input name
        const safeName = DOMPurify.sanitize(name);
