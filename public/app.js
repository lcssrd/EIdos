(function() {
    ...
    function setupEventListeners() {
        ...
        // [NOUVEAU] Bibliothèque Publique
        const publicLibBtn = document.getElementById('public-library-btn');
        if (publicLibBtn) {
            publicLibBtn.addEventListener('click', patientService.openPublicLibrary);
        }
        const publicLibCloseBtn = document.getElementById('public-library-close-btn');
        if (publicLibCloseBtn) {
            publicLibCloseBtn.addEventListener('click', uiService.hidePublicLibraryModal);
        }
        const publicLibSearch = document.getElementById('public-library-search');
        if (publicLibSearch) {
            publicLibSearch.addEventListener('input', (e) => {
                uiService.filterPublicLibrary(e.target.value);
            });
        }
        // Délégation d'événement pour le chargement depuis la biblio publique
        const publicLibList = document.getElementById('public-library-list-container');
        if (publicLibList) {
            publicLibList.addEventListener('click', (e) => {
                const loadBtn = e.target.closest('.load-public-btn');
                if (loadBtn) {
                    const id = loadBtn.dataset.patientId;
                    const name = loadBtn.dataset.patientName;
                    patientService.loadPublicCaseIntoCurrentPatient(id, name);
                }
            });
        }
