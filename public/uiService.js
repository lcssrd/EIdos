(function() {
    ...
    // [NOUVEAU] Variables Bibliothèque Publique
    let publicLibraryModal, publicLibraryBox, publicLibraryListContainer;
    ...
    function applyPermissions(userPermissions) {
        ...
        if (publicLibBtn) {
            if (userPermissions.isStudent) {
                publicLibBtn.style.display = 'none';
            } else {
                publicLibBtn.style.display = 'inline-flex';
            }
        }
    ...
    // [NOUVEAU] Fonctions Bibliothèque Publique
    function showPublicLibraryModal(patients) {
        if (!publicLibraryListContainer) return;
        
        publicLibraryListContainer.innerHTML = '';
        
        if (!patients || patients.length === 0) {
            publicLibraryListContainer.innerHTML = `
                <div class="col-span-full flex flex-col items-center justify-center py-12 text-gray-500">
                    <i class="fas fa-inbox text-4xl mb-3 text-gray-300"></i>
                    <p>Aucun cas public disponible pour le moment.</p>
                </div>`;
        } else {
            // Tri alphabétique
            patients.sort((a, b) => a.sidebar_patient_name.localeCompare(b.sidebar_patient_name));
            
            let html = '';
            patients.forEach(p => {
                const ownerName = p.user ? (p.user.login || p.user.email) : 'Anonyme';
                const date = new Date(p.updatedAt).toLocaleDateString();
                
                html += `
                <div class="bg-white border border-gray-200 rounded-xl p-4 shadow-sm hover:shadow-md transition-all flex flex-col justify-between h-full public-case-card group" data-search-term="${p.sidebar_patient_name.toLowerCase()} ${ownerName.toLowerCase()}">
                    <div>
                        <div class="flex justify-between items-start mb-2">
                            <h4 class="font-bold text-gray-800 text-lg line-clamp-1" title="${p.sidebar_patient_name}">
                                <i class="fas fa-user-injured text-teal-600 mr-2"></i>${p.sidebar_patient_name}
                            </h4>
                        </div>
                        <div class="text-sm text-gray-500 mb-4 space-y-1">
                            <p><i class="fas fa-user-md mr-1.5 w-4 text-center"></i> ${ownerName}</p>
                            <p><i class="far fa-clock mr-1.5 w-4 text-center"></i> ${date}</p>
                        </div>
                    </div>
                    <button type="button" class="load-public-btn w-full py-2 px-4 bg-gray-50 text-gray-700 font-medium rounded-lg border border-gray-200 hover:bg-teal-50 hover:text-teal-700 hover:border-teal-200 transition-all flex items-center justify-center group-hover:bg-teal-600 group-hover:text-white group-hover:border-transparent" 
                            data-patient-id="${p.patientId}" 
                            data-patient-name="${p.sidebar_patient_name}">
                        <i class="fas fa-download mr-2"></i> Charger le cas
                    </button>
                </div>
                `;
            });
            publicLibraryListContainer.innerHTML = html;
        }

        publicLibraryModal.classList.remove('hidden');
        setTimeout(() => {
            publicLibraryBox.classList.remove('scale-95', 'opacity-0');
            document.getElementById('public-library-search').focus();
        }, 10);
    }

    function hidePublicLibraryModal() {
        if(!publicLibraryBox) return;
        publicLibraryBox.classList.add('scale-95', 'opacity-0');
        setTimeout(() => {
            publicLibraryModal.classList.add('hidden');
            document.getElementById('public-library-search').value = ''; // Reset search
        }, 200);
    }

    function filterPublicLibrary(query) {
        if (!publicLibraryListContainer) return;
        const term = query.toLowerCase();
        const cards = publicLibraryListContainer.querySelectorAll('.public-case-card');
        let count = 0;
        
        cards.forEach(card => {
            const searchText = card.dataset.searchTerm;
            if (searchText.includes(term)) {
                card.style.display = 'flex'; // 'flex' car c'est la classe CSS du conteneur
                count++;
            } else {
        ...
        let noResultMsg = document.getElementById('public-lib-no-result');
        if (count === 0 && term !== '') {
            if (!noResultMsg) {
                noResultMsg = document.createElement('div');
                noResultMsg.id = 'public-lib-no-result';
                noResultMsg.className = 'col-span-full text-center py-8 text-gray-500 italic';
                noResultMsg.textContent = 'Aucun résultat correspondant.';
                publicLibraryListContainer.appendChild(noResultMsg);
            }
        } else if (noResultMsg) {
            noResultMsg.remove();
        }
    ...
    function updatePancarteChart() {
        ...
        const zoomText = document.getElementById('zoom-level-pancarte-text');
        ...
        const updateScrollbarAndText = (chart) => {
            ...
            const isFullyVisible = (max - min) >= (limitMax - limitMin - 0.1); 
        ...
        scrollbar.oninput = () => {
            ...
            pancarteChartInstance.update('none'); 
        };
    }
    ...
    // --- WIDGET ÉTUDIANTS CONNECTÉS ---
    let widgetDrag = { active: false, currentX: 0, currentY: 0, initialX: 0, initialY: 0, xOffset: 0, yOffset: 0 };

    function initStudentWidget() {
        const widget = document.getElementById('connected-students-widget');
        const header = document.getElementById('students-widget-header');
        const toggleBtn = document.getElementById('students-widget-toggle');

        if (!widget || !header) return;

        // Gestion du repli
        toggleBtn.addEventListener('click', (e) => {
            e.stopPropagation(); // Empêche le drag de démarrer
        ...
        function dragStart(e) {
            if (e.target === toggleBtn || e.target.closest('#students-widget-toggle')) return;
            
            widgetDrag.initialX = e.clientX - widgetDrag.xOffset;
            widgetDrag.initialY = e.clientY - widgetDrag.yOffset;
        ...
        function dragEnd(e) {
            widgetDrag.initialX = widgetDrag.currentX;
            widgetDrag.initialY = widgetDrag.currentY;
            widgetDrag.active = false;
            if(header) header.style.cursor = "move";
        }

        function drag(e) {
            if (widgetDrag.active) {
                e.preventDefault();
            
                widgetDrag.currentX = e.clientX - widgetDrag.initialX;
                widgetDrag.currentY = e.clientY - widgetDrag.initialY;

                widgetDrag.xOffset = widgetDrag.currentX;
                widgetDrag.yOffset = widgetDrag.currentY;

                setTranslate(widgetDrag.currentX, widgetDrag.currentY, widget);
            }
        }
    ...
    function updateConnectedStudentsList(students) {
        ...
        // Afficher le widget si ce n'est pas déjà fait
        widget.classList.remove('hidden');

        badge.textContent = students.length;

        if (students.length === 0) {
            list.innerHTML = '<li class="text-xs text-gray-500 text-center italic py-2">Aucun étudiant connecté</li>';
            return;
        }
        ...
        students.forEach(s => {
            ...
            <li class="student-item">
                ...
                <span class="text-sm font-medium text-gray-700 truncate">${DOMPurify.sanitize(s.login)}</span>
            </li>`;
        });
        list.innerHTML = html;
    }
    ...
    window.uiService = {
        ...
        showPublicLibraryModal, hidePublicLibraryModal, filterPublicLibrary,
        initStudentWidget, updateConnectedStudentsList
    };
})();